...
class MagneticIntegral(Problem.LinearProblem):
    ...
    def Intrgl_Fwr_Op(self, m=None, magType='H0'):
        ...
        job = Forward(
                ...
                model=self.model, Mxyz=self.Mxyz,
                P=self.ProjTMI, parallelized=self.parallelized,
                n_chunks=self.n_chunks, components=self.survey.components,
...
def calcRow(
    ...
    for comp in components:
        ...
        if (comp == "dbx_dx") or ("dbz_dz" in components):
            ...
            dbx_dx[0, nC:2*nC] = (dx1 / (r1 * arg5 + eps) - dx2 / (r2 * arg10 + eps) +
                               dx2 / (r3 * arg15 + eps) - dx1 / (r4 * arg20 + eps) +
                               dx2 / (r5 * arg25 + eps) - dx1 / (r6 * arg30 + eps) +
                               dx1 / (r7 * arg35 + eps) - dx2 / (r8 * arg40) + eps)
        ...
        if (comp == "dby_dy") or ("dbz_dz" in components):
            ...
            dby_dy[0, 0:nC] = (dy2 / (r1 * arg5 + eps) - dy2 / (r2 * arg10 + eps) +
                            dy2 / (r3 * arg15 + eps) - dy2 / (r4 * arg20 + eps) +
                            dy1 / (r5 * arg25 + eps) - dy1 / (r6 * arg30 + eps) +
                        dy1 / (r8 * arg35 + eps) - dy1 / (r8 * arg40 + eps))
